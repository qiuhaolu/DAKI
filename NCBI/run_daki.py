...
args = {
    ...
    'task_name': 'binary',
    'output_dir': '/data/qlu1/DAKI/outputs/NCBI/',
    'cache_dir': 'cache/',
    'do_train': True,
    'do_eval': True,
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            if(attention_scores_prob_all is None):
                ...
                #print(attention_scores_prob_all.shape)
            
            #print(attention_scores_prob_batch)
            #print(attention_scores_prob_batch.shape) #[3 layers,batchsize,3 adapters]
            #exit()

            last_hidden_states = outputs[0]#[0]
            logits = towerModel(last_hidden_states)
            labels = batch[3]
            tmp_eval_loss = criterion(logits.view(-1, 3), labels.view(-1))
...
def final_test_checkpoint(mode, withadapters, checkpoint):

    results = {}
    result_list = []
    checkpoints = ['outputs/checkpoint-'+checkpoint]
