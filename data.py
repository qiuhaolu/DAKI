...
class UMLSDataset(Dataset):
    def __init__(self, skiprate, tokenizer, rel_vocab, rel_fine_vocab, concepts, relations, concept_index=None, debug=False):
        ...
        self.cui2defi = {}
        with open('UMLS/dataset/MRDEF.RRF') as f:
            for line in f:
                line = line.split('|')
                #cui = line[0]
                #defi = line[5]
...
def batchfn(data, device):
    ...
    triple_pos_ids_s = torch.tensor(triple_pos_ids_s).to(device)
    triple_pos_mask_s = torch.tensor(triple_pos_mask_s).to(device)
    triple_neg_ids_s = torch.tensor(triple_neg_ids_s).to(device)
    triple_neg_mask_s = torch.tensor(triple_neg_mask_s).to(device)
    output = {
...
def batchfn_disease(data, device):
    ...
    token_auxed_passages = torch.tensor(token_auxed_passages).to(device)
    token_auxed_passages_ori = torch.tensor(token_auxed_passages_ori).to(device)
...
class dataset_sgrouping(Dataset):
    def __init__(self, tokenizer, fine_grained):
        ...
        self.cui2text = {}
        with open('UMLS/dataset/concepts.txt') as f:
            for line in f:
                CUI, name, name_seg = line.strip().split('\t')
                #if(CUI not in linker.kb.cui_to_entity):
                #    continue # if missing text, skip
        ...
        temptid = 0
        self.tui2sgroupid = {}
        with open('UMLS/dataset/SemGroups.txt') as f:
            for line in f:
                line = line.split('|')
                #cui = line[0]
                #defi = line[5]
        ...
        self.cui2sgroupid = {}
        self.cui2styid = {}
        with open('UMLS/dataset/MRSTY.RRF') as f:
            for line in f:
                line = line.split('|')
                #cui = line[0]
                #defi = line[5]
...
def batchfn_sgrouping(data, device):
    ...
    defi_input_ids_batch = torch.tensor(defi_input_ids_batch).to(device)
    defi_mask_batch = torch.tensor(defi_mask_batch).to(device)
    defi_label_batch = torch.tensor(defi_label_batch).to(device)
    return {
